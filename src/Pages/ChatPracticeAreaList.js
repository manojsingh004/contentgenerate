...
const ChatPracticeAreaList = () => {
  ...
  const deleteDocument = async (id) => {
    try {
      ...
      setDocuments((prevDocuments) => prevDocuments.filter((doc) => doc.id !== id)); // Update state to remove the deleted document
    } catch (error) {
            ...
            {documents.map((doc, index) => (
              <tr key={index}>
                ...
                <td>
                  ...
                  <Button style={{background:"transparent",border:'0',padding:'0'}} onClick={() => deleteDocument(doc.id)}>
                  <svg width="20" height="20" viewBox="0 0 24 25" fill="none" xmlns="http://www.w3.org/2000/svg">
